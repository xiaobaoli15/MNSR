function [X,E,obj,err,iter,Q] = lrsr(A,B,lambda1,lambda2,opts)
...
DEBUG = 0;
loss = 'l21';
intra_epsion=0.1;%0.05
intra_epsion_1=1;
labmda3=0.00001;%0.01
if ~exist('opts', 'var')
    opts = [];
end    
...
%%
                Q_W=[];
                for i=1:size(A,2)
                    Q_i=exp(sqrt(sum((repmat(A(:,i),1,size(B,2))-B).^2))/intra_epsion);
                    %Q_i=sqrt(sum((repmat(A(:,i),1,size(B,2))-B).^2));
                    Q_i=Q_i./max(Q_i);
                    Q_W=[Q_W Q_i'];                   
                end
                Q= Q_W;
...
for iter = 1 : max_iter
    ...
    [Z,nuclearnormZ] = prox_nuclear(X+Y2/mu,0/mu);%0.5
    %Z=1/(2+mu)*(Y2+mu*X);
    ...
    %X = invBtBI*(B'*(Y1/mu-E)+BtA-(Y2+Y3)/mu+Z+J);
     X = invBtBI*(B'*(Y1/mu-E)+BtA-(Y2+Y3)/mu+Z+J-Q*labmda3/mu);
    X=max(X,0); 
    
    %Q=1./exp((abs(X))+intra_epsion_1);
